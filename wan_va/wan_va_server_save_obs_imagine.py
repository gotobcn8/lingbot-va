...
class VA_Server:
    ...
    def _to_uint8_image(image):
        ...
        if image.dtype != np.uint8:
            if np.issubdtype(image.dtype, np.floating):
                ...
                image = image * 255.0 if max_value <= 1.0 else image
    ...
    def _save_obs_images(self, obs, step_dir):
        images = obs.get('obs', [])
        if not isinstance(images, list):
            images = [images]

        obs_dir = os.path.join(step_dir, 'obs')
        os.makedirs(obs_dir, exist_ok=True)
        for history_id, image_dict in enumerate(images):
            if not isinstance(image_dict, dict):
                image = self._to_uint8_image(image_dict)
                Image.fromarray(image).save(os.path.join(obs_dir, f'history_{history_id:03d}.png'))
                continue

            for cam_key, image_value in image_dict.items():
                image = self._to_uint8_image(image_value)
                cam_name = self._safe_name(cam_key)
                Image.fromarray(image).save(
                    os.path.join(obs_dir, f'history_{history_id:03d}_{cam_name}.png')
                )

    def _save_imagine_images(self, latents, step_dir):
        imagine_dir = os.path.join(step_dir, 'imagine')
        os.makedirs(imagine_dir, exist_ok=True)
        decoded_video = self.decode_one_video(latents.detach(), 'np')[0]
        for frame_id, frame in enumerate(decoded_video):
            image = self._to_uint8_image(frame)
            Image.fromarray(image).save(os.path.join(imagine_dir, f'frame_{frame_id:03d}.png'))

    def _save_step_obs_and_imagine(self, obs, latents, frame_st_id):
        if not getattr(self.job_config, 'save_obs_imagine_images', True):
            return

        step_dir = os.path.join(self.exp_save_root, f'step_{frame_st_id:06d}')
        os.makedirs(step_dir, exist_ok=True)
        self._save_obs_images(obs, step_dir)
        self._save_imagine_images(latents, step_dir)
    ...
    def _infer(self, obs, frame_st_id=0):
        ...
        self._save_step_obs_and_imagine(obs, latents, frame_st_id)
...
def run(args):    
    ...
    config.save_obs_imagine_images = not args.disable_save_obs_imagine_images
...
def main():
    ...
    parser.add_argument(
        "--disable_save_obs_imagine_images",
        action="store_true",
        help="disable per-step obs and imagine png saving",
    )
