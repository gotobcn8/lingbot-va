...
repr_path = '/data/.cache/datasets/lerobot/robotwin/clean_tiny/open_microwave-demo_clean_collect_200-50/'
...
def _load_images(input_dir: str, device: torch.device) -> List[Dict]:
    ...
    fnames = sorted(
        ...
        if f.lower().endswith((".pth")) and "_vis" not in f
    )
...
def parse_args():
    data_root = '/data/.cache/datasets/lerobot/robotwin/clean_tiny/open_microwave-demo_clean_collect_200-50/'
    ...
    p.add_argument("--repr_dir", type=str, default=data_root,
                   help="Directory containing scenes (each subfolder is a scene)")
